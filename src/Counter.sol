...
contract OverflowICO is Ownable, ReentrancyGuard, LinearVesting {
    ...
    IERC20 public immutable buyerToken;
    ...
    function calculateEmission(uint256 value) internal view returns (uint256) {
        return (value * emissionPerEther) / 10 ** salesTokenDecimals;
    }
    ...
    function _updateEmission() internal {
        ...
        if (totalCommitments > 0) {
            ...
            emissionPerEther += (emission * 10 ** salesTokenDecimals) / totalCommitments;
        }
    ...
    constructor(
        ...
        require(tokensToRaise > 0, "Ethers to raise should be greater than 0");
        require(tokensToRaise > _refundThreshold, "Ethers to raise should be greater than refund threshold");
        ...
        salesTokenDecimals = IERC20(salesToken).decimals();
        tokensToSell = _tokensToSell;
        ethersToRaise = tokensToRaise;
    ...
    // 入金
    function commit(bytes calldata sig, uint _amount) external payable nonReentrant {
        ...
        // これを叩くことによってcalculateEmissionの量が増える
        // からmissedEmissionsの量が増える＝遅いほど貰える量が減る
        _updateEmission();
        ...
        // 額の制限
        require(
            minCommit <= commitments[msg.sender] + _amount && commitments[msg.sender] + _amount <= maxCommit,
            "Commitment amount is outside the allowed range."
        );
        // どうせエラー出るから↓いらない
        // require(IERC20(_buyerToken).allowance(msg.sender, address(this)) > _amount, "over allowance");
        IERC20(buyerToken).transferFrom(msg.sender, address(this), _amount);

        // commitments = いくらつっこんだ？
        commitments[msg.sender] += _amount;
        totalCommitments += _amount;
        // 遅くなったことによる損失額
        missedEmissions[msg.sender] += calculateEmission(_amount);
    ...
    // claim2の前にこれが必要らしい
    // commitmentsが上限突破したら一部を、そうじゃなければ全額を消費した扱いにして残りのethを返金
    // するのと、排出量をセットする
    function claim() external nonReentrant returns (uint256, uint256, uint256) {
        ...
        if (totalCommitments >= refundThreshold) {
            ...
            //なんでカッコつけてるのか知らんけど売る用のトークン枚数 * 上限額に対する自分が支払ったethの割合
            //ただしmissedEmission考慮せず
            //これは見た通りeth上限と自分の割合で配られる
            uint256 tokensToReceive = (tokensToSell * ethersToSpend) / ethersToRaise;
            //考慮した版？
            //こっちはtotalEmissionとtotalCommitmentを元に計算されるみたい
            //totalEmissionは先着の人の分の利益っぽいね
    ...
    function finish() external onlyOwner {
        ...
        if (totalCommitments >= refundThreshold) {
            IERC20(buyerToken).transfer(owner(), Math.min(ethersToRaise, totalCommitments));
            ...
            salesToken.safeTransfer(owner(), tokensToSell + totalEmission);
        }
    }
